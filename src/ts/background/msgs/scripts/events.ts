    ...
    (msg: t.Any): t.Any =>
        err(() => {
            ...
            if (msg_str === 'get_settings') {
                return ext
                    .storage_get()
                    .then((response: t.AnyRecord | undefined) => response)
                    .catch((error_obj: i_error.ErrorObj) => show_err_ribbon(error_obj, 'aer_1114'));
            }
            ...
            if (msg_str === 'update_settings') {
                if (n(msg.update_instantly) && msg.update_instantly) {
                    return s_data.Manipulation.update_settings({
                        ...
                        .then(() => true)
                        .catch((error_obj: i_error.ErrorObj) =>
                            show_err_ribbon(error_obj, 'aer_1127'),
                        );
                }
            ...
            if (msg_str === 'react_to_change') {
                return s_side_effects.SideEffects.react_to_change()
                    ...
                    .catch((error_obj: i_error.ErrorObj) => show_err_ribbon(error_obj, 'aer_1105'));
            }

            if (msg_str === 'get_window_focus_state') {
                return s_reload.Popup.get_window_focus_state()
                    .then((response: t.Any) => response)
                    .catch((error_obj: i_error.ErrorObj) => show_err_ribbon(error_obj, 'aer_1150'));
            }
